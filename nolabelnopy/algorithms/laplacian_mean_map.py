...
class BaggedKFold(_BaseKFold):
    ...
    def __init__(self, bag_id, n_folds=3, indices=None, shuffle=False,
        ...
        super(BaggedKFold, self).__init__(n, n_folds, indices,
        ...
        if self.n_folds > min_bag_size:
            raise ValueError(("The least populated bag has only %d members,"
                              "which is less than n_folds=%d. Instances will"
                              "be resampled."
...
#                            .format(min_bag_size, self.n_folds)), Warning)
        ...
        bag_to_x = np.array([[i for i, x in enumerate(bag_id) if x == b]
                            for b in unique_bag])
...
class LaplacianMeanMap:
    ...
    def fit(self, X, pi):
        ...
        # set the Laplacian
        L = np.identity(2*self.n) * self.epsilon + \
            sp.linalg.block_diag(self.La, self.La)
        ...
        mean_op = np.sum(np.array([(p[j] * (pi[j] * b_plus[j] -
                         (1-pi[j]) * b_minus[j])) for j in range(self.n)]),
                         axis=0)
    ...
    def predict(self, X):
        return np.sign(sp.special.expit(2 * np.dot(X.iloc[:],
                       self.theta)) - .5)
...
#        self.dataframe_result.columm = ['alpha', 'L_normed', 'gamma', 'D_w',
#                                        'eps', 'd', 'sigma', 'accuracy']
    ...
    def fit(self, X, pi):
        ...
        # set the proportions matrix --this does not change as we do not
        # know the labels
        Pi = np.transpose(np.hstack((np.diag(pi), np.diag(1-pi))))
...
#       for d, sigma, L_normed in product(self.ds, self.sigmas, self.L_normed):
        for sigma, L_normed in product(self.sigmas, self.L_normed):
            ...
            La = [laplacian_matrix(B[i], d, normed=L_normed)
            ...
            for w_type in self.w_types:
                ...
                for gamma, alpha in product(self.gammas, self.alphas):
                    ...
                    for v, (idx_train, idx_validation) in enumerate(folds):
                        ...
                        self._fit(X.iloc[idx_train], Pi, B[v], p, D_w, L[v],
                                  gamma, alpha)
                        s = self.predict(X.iloc[idx_validation])
                        score += 1/self.cv * proportions_abs_err(X, pi, p, s)
